...
def run_rag_with_progress(qna, output_file, single_entry_processor_func, **processor_kwargs):
    """
    ...
    if completed_entries:
        LOGGER.info(f"Resuming from entry {start_index} (found {len(completed_entries)} completed entries)")
    else:
    ...
    if missing_indices:
        LOGGER.warning(f"Found {len(missing_indices)} gaps in completed entries that will be reprocessed: {missing_indices}")
...
# Assisted by Cursor with claude-4-sonnet
def run_reference_rag(qna, generator_model, idx, output_file, number_of_search_results=20, number_of_selected_results=5):
    # Doc string assisted by watsonx Code Assistant
    """
...
def run_reference_rag_single_entry(entry, generator_model, idx, number_of_search_results=20, number_of_selected_results=5):
    """
    Core RAG logic for processing a single Q&A entry.
...
def run_evaluation_of_questions_without_reference_answers(datasets_without_reference_answers, evaluator_llm_for_ragas):
    ...
    for label, data in datasets_without_reference_answers.items():
        ...
        for qna in data:
            ...
            if check_if_answer_is_attempting_to_answer_question(question, response, evaluator_llm_for_ragas):
                scores.append(0)
            else:
                scores.append(1)
...
def report_results_with_significance(results, metrics, subset_of_rows = None):
    ...
    for result_pair in result_pairs:
        ...
        for metric in metrics:
            results0 = results[result_pair[0]] if isinstance(results[result_pair[0]], pd.DataFrame) else results[result_pair[0]].to_pandas()
            results1 = results[result_pair[1]] if isinstance(results[result_pair[1]], pd.DataFrame) else results[result_pair[1]].to_pandas()
    ...
    while True:
        try:
            ...
            if retries <= max_retries:
                LOGGER.error(f"An exception of type {e.__class__.__name__} occurred.  Waiting {delay_between_retries_seconds} seconds to give the servers time to stabilize and then resuming.")
                ...
                LOGGER.error(f"An exception of type {e.__class__.__name__} occurred.  Maximum number of retries exceeded.  Terminating.")
                raise e
...
def parse_llm_relevance_response(llm_response: str) -> bool:
    ...
    if not isinstance(llm_response, str):
        ...
        LOGGER.warning(f"Input to parser was not a string: {type(llm_response)}. Returning False.")
        return False
    ...
    if "yes" in words:
        ...
        LOGGER.warning(f"Warning: LLM response '{llm_response}' did not clearly contain 'yes' or 'no'. Returning False.")
        return False
